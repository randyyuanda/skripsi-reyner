...
<body>
          ...
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-left p-5">
              ...
              <div class="form-group">
                <input id="name" disabled type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" name="name" value="{{ $klien->name }}" required autocomplete="name" autofocus>
              </div>
              <div class="form-group">
                <input id="email" disabled type="email" class="form-control form-control-lg @error('email') is-invalid @enderror" name="email" value="{{ $klien->email }}" required autocomplete="email" placeholder="Email">
              </div>
              <div class="form-group">
              ...
              <div class="mt-3" style="text-align: center">
                <button class="btn btn-block btn-gradient-primary btn-lg font-weight-medium" onclick="updateKlien()">SUBMIT</button>
              </div>
            </div>
          </div>
        </div>
  ...
  <script>
    function updateKlien() {
      ...
      $.ajax({
        type: "POST",
        url: "/api/change-password/{{ $klien->id }}",
        data: formData,
        contentType: false,
        processData: false,
        cache: false,
